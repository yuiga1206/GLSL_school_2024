...
/** ===========================================================================
 * フレームバッファを使ってポストプロセスでなんらかの演出を行う原理は、シェーダ
 * アートのときと基本的には変わりません。
 * テクスチャ座標をまず先に歪めておくことで参照する座標をずらしたり、あるいは、
 * カーソルの位置に応じて変換したりすることで、印象的なインタラクティブ演出を行
 * うことができます。
 * またフォトレタッチソフトのように、彩度を落としてグレイスケール化するなど画面
 * 全体に対して一様にエフェクトを掛けるような処理も、フレームバッファを活用した
 * テクニックの定番と言えます。
 * ========================================================================= */
...
class WebGLApp {
  ...
  constructor() {
    ...
    this.uSaturation = 0.0;   // uniform 変数 saturation 用 @@@
    ...
    pane.addBlade({
      ...
      min: 0.0,
      max: 1.0,
      value: this.uSaturation,
    })
    .on('change', (v) => {
      this.uSaturation = v.value;
    });
  }
  /**
