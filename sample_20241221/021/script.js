...
/** ===========================================================================
 * 比較的王道の演出として、レンズなどで起こる色収差を再現したような、 RGB をずら
 * して描画するテクニックがあります。
 * 一見すごくややこしい物理法則などを駆使しているように見えるかもしれませんが、
 * 実際にはテクスチャ座標をずらしてやることで近似することができます。
 * 単純な算数なので落ち着いてフラグメントシェーダを読み解きましょう。
 * ========================================================================= */
...
class WebGLApp {
  ...
  constructor() {
    ...
    this.uDistortionArea = 0.2;  // uniform 変数 distortionArea 用 @@@
    this.uDistortionScale = 0.2; // uniform 変数 distortionScale 用 @@@
    ...
    pane.addBlade({
      view: 'slider',
      label: 'distortion-area',
      min: 0.0,
      max: 1.0,
      value: this.uDistortionArea,
    })
    .on('change', (v) => {
      this.uDistortionArea = v.value;
    });
    pane.addBlade({
      view: 'slider',
      ...
      min: 0.0,
      max: 1.0,
      value: this.uDistortionScale,
    })
    .on('change', (v) => {
      this.uDistortionScale = v.value;
    });
  }
  /**
  ...
  async load() {
    ...
    this.postShaderProgram = new ShaderProgram(this.gl, {
      ...
      uniform: [
        'textureUnit',
        'brightness',
        'distortionArea', // 歪み範囲係数 @@@
        'distortionScale', // 歪み量係数 @@@
        'mouse',
      ],
      type: [
        'uniform1i',
  ...
  renderPostProcess() {
    ...
    this.postShaderProgram.setUniform([
      0,
      this.uBrightness,
      this.uDistortionArea, // 歪み範囲係数 @@@
      this.uDistortionScale, // 歪み量係数 @@@
      this.uMouse,
    ]);
