 ...
 * ※乱数生成器そのものを自作する必要がある
 *
 * ここでは、最もシンプルで簡素な乱数生成器を実装し、生成した乱数を活用して砂嵐
 * のような効果を適用しています。
 * それ以外にも、ヴィネットやサイン波による走査線などを組み合わせてありますので
 * フラグメントシェーダのコードを確認してみましょう。
 * ========================================================================= */
...
class WebGLApp {
  ...
  constructor() {
    ...
    this.uFrequency = 500.0;  // uniform 変数 frequency 用 @@@
    ...
    pane.addBlade({
      ...
      min: 1.0,
      max: 1000.0,
      value: this.uFrequency,
    })
    .on('change', (v) => {
      this.uFrequency = v.value;
    });
  }
  /**
