...
/** ===========================================================================
 * バリューノイズのような連続性のあるノイズに Simplex noise があります。
 * ここではシンプレックスノイズを使って、トランジションエフェクトの一例を実装し
 * ています。
 * シンプレックスノイズは滑らかな濃淡があり、WebGL の世界ではよく用いられます。
 * ただしライセンス的に Ashima Arts が公開しているものは大丈夫だけど、オリジナル
 * のシンプレックスノイズは駄目、みたいなことがあるようなので、使う際はコメント
 * で Copyright 表記を記述しておくなど取り扱いには十分に気をつけましょう。
 * 変更点のほとんどは、フラグメントシェーダ側です。JavaScript からは uniform 変
 * 数周りの手続きだけが変更されています。
 * ========================================================================= */
...
class WebGLApp {
  ...
  constructor() {
    ...
    this.uIntensity = 0.01;   // uniform 変数 intensity 用
    this.uContrast = 2.0;     // uniform 変数 contrast 用
    this.uScale = 3.0;        // uniform 変数 noiseScale 用 @@@
    this.uOffset = 0.2;       // uniform 変数 noiseOffset 用 @@@
  ...
  renderPostProcess() {
    ...
    this.postShaderProgram.setUniform([
      ...
      this.uIntensity,
      this.uContrast,
      this.uScale, // ノイズのスケール @@@
      this.uOffset, // ノイズのオフセット係数 @@@
      this.uMouse,
    ]);
